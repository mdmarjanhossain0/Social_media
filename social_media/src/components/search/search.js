  ...
  useEffect(() => {
    ...
    return (
      <div className='container'>
        {list.map(item =>
          <>
            <Card className="friend-card" key={item.id} onClick={() => navigateChat(item)}>
              ...
              <div className='text-info'>
                ...
                {/* {item.is_friend ?
                  <>
                    <button type="button" className="btn btn-primary" onClick={() => friendFunctions(item.pk, 3)}>Chat</button>
                  </> :
                  <></>
                } */}
              </div>
              ...
              <div className='card-button'>
                {item.is_friend ? 
                  <>
                    <button type="button" className="btn btn-primary" onClick={() => friendFunctions(item.pk, 3)}>Unfriend</button>
                  </>  :
                  <>
                    {item.is_requested ? 
                      <button type="button" className="btn btn-primary" onClick={() => friendFunctions(item.pk, 2)}>Accept Request</button> :
                      <>
                        {item.is_request_send ? <button type="button" className="btn btn-primary">Pending</button> :
                        <button type="button" className="btn btn-primary" onClick={() => friendFunctions(item.pk, 1)}>Add</button>}
                      </>
                   }
                  </>}
              </div>
